...
public class TheWorldRenderer extends StandEntityRenderer<TheWorldEntity> {
    public TheWorldRenderer(final EntityRendererProvider.Context context) {
        super(context, new TheWorldModel());
    }

    @Override
    public void actuallyRender(final PoseStack poseStack, final TheWorldEntity animatable, final BakedGeoModel model, final RenderType renderType, final MultiBufferSource bufferSource, final VertexConsumer buffer, final boolean isReRender, final float partialTick, final int packedLight, final int packedOverlay, final float red, final float green, final float blue, final float alpha) {
