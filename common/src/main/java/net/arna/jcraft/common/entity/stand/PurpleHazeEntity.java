...
public final class PurpleHazeEntity extends AbstractPurpleHazeEntity<PurpleHazeEntity, PurpleHazeEntity.State> {
    ...
    public void desummon() {
        if (toEvolve && hasUser()) {
            ComponentPlatformUtils.getStandData(getUserOrThrow()).setType(StandType.PURPLE_HAZE_DISTORTION);
    ...
    public void freshKill(@Nullable LivingEntity entity) {
        ...
        if (!StandType.isNone(ComponentPlatformUtils.getStandData(entity).getType()))
            flowerable = true;
    }
    ...
    protected ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (player == getUser() && stack.isIn(ItemTags.FLOWERS)) {
            ...
            if (!getWorld().isClient()) {
                ...
                final CommonPhComponent ph = ComponentPlatformUtils.getPhData(player);
                ph.increaseLevel();
                if (ph.getLevel() >= 5) {
                    ph.resetLevel();
                    toEvolve = true;
