...
public class HitPropertyCapability extends CommonHitPropertyComponentImpl implements JCapability {
    ...
    public void sync(Entity entity) {
        super.sync(entity);
        if (entity != null) {
            HitPropertyCapability.syncEntityCapability(entity);
        }
    }

    private static void syncEntityCapability(Entity entity) {
        if (entity instanceof LivingEntity living) {
            JNetworkingForge.sendPackets(living, HIT_S2C, HIT_C2S, getCapability(living));
        }
    }
    ...
    public static void initNetwork(){
        ...
        NetworkManager.registerReceiver(NetworkManager.Side.C2S, HIT_C2S, (buf, context) -> {
            ...
            if (context.getPlayer().level() != null) {
                HitPropertyCapability.getCapabilityOptional(context.getPlayer().level().getEntity(id)).ifPresent(c -> c.deserializeNBT(nbt));
            }
        });
    }
}